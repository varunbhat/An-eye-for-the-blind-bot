...
def radius_set(event, x, y, flag, param):
    f = open('radius.txt','w');
    f.write(str(int((x*x+y*y)**.5)))
...
dialatecount = 0
thresholdrange = [(60,0,50,0),(120,10,100,0) ]
...
cv.ShowImage('threshold', imageTreshold)
while 1:
    cv.SetMouseCallback("threshold", radius_set, 0);
    keyPressed = cv.WaitKey(10000)
    if cv.WaitKey()==27:
        break
...
#    difflistbetweenspaces = []
    ...
    y = open('differlist.txt','w')
    for i in difflistfromCenter:
        y.write(str(i)+'\n')
